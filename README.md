...
| 直闪职感 | 都市街拍 | 高山云海（川内伦子） |
|:---:|:---:|:---:|
| ![直闪职感](showcase/flash-office-20260720.png) | ![都市街拍](showcase/urban-street-20260720.png) | ![高山云海](showcase/mountain-cloud-kawase.png) |
| CCD 硬阴影、皮肤反光、现场感 | 城市自然光、胶片颗粒、生活感 | 川内伦子空气感、明亮柔焦、淡雅色调
...
| **视频分镜 (NextShotPhoto)** | 逐镜尾帧 prompt + 视频生成提示词 | 分镜链式生产，运镜变换矩阵推导尾帧，人物/产品/氛围跨镜锁定。含桥接帧决策引擎（轻量/重量二级判断）、空间几何锁定、身份锚点、参考图链路（首帧→桥接帧→尾帧三步强一致） |
...
→ [可选] 视频分镜 NextShotPhoto →
  ...
  变换矩阵推导尾帧→桥接帧决策（轻量/重量）→
  参考图链路生成 → 输出尾帧 prompt + 视频生成提示词→
  循环下一镜
```
...
| **三重锁定防漂移** | 字面量焊接 + 变体隔离 + 首帧视觉锚定 + **参考图链路**（首帧→桥接帧→尾帧逐帧参考），人物/产品/氛围跨镜一致 |
| **桥接帧决策引擎** | Delta 分值自动判定：0-2 直接输出 / 3-5 轻量桥接（文本过渡路径）/ ≥6 重量桥接（建议生成中间状态图）。空间几何锁定 + 身份锚点防止桥接帧漂移 |
| **负面约束引擎** | 运镜感知的约束增益/衰减（手持时自动删除「构图平稳」，极特写时增强手指约束） |
| **双产物输出** | 每镜输出尾帧图片 prompt（给 DragonCode / gpt-image-2）+ 视频生成提示词（给 Seedance/Kling/Wan）。🚫 禁止使用带水印的生成工具（ImageGen 混元模型） |
...
首帧结构化字段 + 用户输入（运镜选择 + 动作描述）
  → 运镜变换矩阵（字段级变换规则）
  → 桥接帧决策引擎（Delta 分值判定是否需要中间过渡）
  → 尾帧 prompt（锁定域不变 + 变体域更新）
  → 参考图链路生成（首帧→桥接帧→尾帧三步递进）
  → 视频生成提示词（运镜描述 + 动作 + 转场 + 时长）
```
...
### 桥接帧效果 — 「田野胡旋舞 → 山顶云海」3 帧序列

下面是一组 Delta 总分 8 的重量桥接场景，展示了桥接帧决策引擎的落地效果。运镜要求：首帧中景逆光旋转（田野）→ 尾帧背面高机位极特写（山顶云海），中间景别差 3 级、机位差 180°、场景类型改变：

| Shot 1 — 首帧（田野胡旋舞） | Shot 2a — 桥接帧（过渡状态） | Shot 2 — 尾帧（山顶背面极特写） |
|:---:|:---:|:---:|
| ![首帧](showcase/bridge-test-shot1.png) | ![桥接帧](showcase/bridge-test-bridge.png) | ![尾帧](showcase/bridge-test-tail.png) |
| 王家卫电影感，中景逆光仰拍，胡旋舞 | 近景侧面平视，收舞转身，场景过渡 | 背面高机位极特写，云海远山，披帛飘入 |

**图片生成链路**：首帧图 →(作为参考图)→ 桥接帧图 →(作为参考图)→ 尾帧图。生成工具为 DragonCode（gpt-image-2 管线，无水印），使用 `input_fidelity: high` 高保真参数。参考图将人物服装、发髻、披帛等视觉特征逐帧传递，防止中间过渡帧漂移。

**桥接帧推导规则**：
- 景别 = 首帧（中景）与尾帧（极特写）的中间值 → 近景
- 机位 = 首帧（逆光仰拍）与尾帧（背面）的中间值 → 侧面平视
- 动作 = 过渡状态：「人物停止旋转，披帛缓缓落下，身体正在转身」
...
├── showcase/                # 生成效果图 + 分镜故事板（雨末茶室3镜 + 桥接帧3帧序列）
├── scripts/
│   └── verify.sh            # Skill 健康检查脚本（20 项）
├── test-prompts.json        # 9 个标准化测试用例（含 nsp-001～nsp-003 分镜测试）
├── dragoncode/              # DragonCode 图片生成 skill（gpt-image-2 管线，无水印）
├── README.md                # 本文件
└── LICENSE                  # MIT
```
...
PerfectPhoto 已通过多个完整流程的实操验证（ImageGen + DragonCode GPT-Image-2），并完成了带参考图链路的桥接帧端到端测试：
...
| NextShotPhoto 桥接帧 | 首帧→桥接帧→尾帧（景别差3级+机位差180°） | Delta 分值判定→插入桥接帧→参考图链路生图 | ✅ 三帧人物/服装一致，无水印 |
| 王家卫+川内伦子双大师 | 两套人物锁定+两套大师风格 | 分两次出图，人物/场景/光线各不一致 | ✅ 互不干扰 |
