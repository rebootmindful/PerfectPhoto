...
| **双产物输出** | 每镜输出尾帧图片 prompt（给图片生成工具）+ 视频生成提示词（给 Seedance/Kling/Wan）。🚫 注意选择无水印且支持参考图的工具 |
...
**图片生成链路**：首帧图 →(作为参考图)→ 桥接帧图 →(作为参考图)→ 尾帧图。每步生成时传入上一镜图片作为参考，使用高保真参数锁定视觉特征。参考图将人物服装、发髻、披帛等视觉特征逐帧传递，防止中间过渡帧漂移。
...
PerfectPhoto 已通过多个完整流程的实操验证（图片生成工具），并完成了带参考图链路的桥接帧端到端测试：
...
| 图生图变体 | 原图→3个姿势变化 | 背对背/骑车载她/并肩远望 | ✅ 人物一致 |
